...
/*
  ...
  （※ Wix購入履歴との照合による正式な「有効ブレンド数5種類以上」判定は、
     運営者が管理シート上で確定させる運用としています。README参照）
*/
...
const STAGE_META = {
  1: { label: "STAGE 1｜基礎を整える" },
  2: { label: "STAGE 2｜提案の幅を広げる" },
  3: { label: "STAGE 3｜選ばれるサロンへ" },
};
...
function buildGrowText(answers, levels, stageResult) {
  ...
  if (levels.range <= 1) {
    parts.push("<p>まずは今あるブレンドを、自信を持って届けるところから始められます。よく相談されるお悩みや、これから届けたい体験から、次に加えるブレンドを考えてみましょう。</p>");
  } else {
